...
void PairMIECut::compute(int eflag, int vflag)
{
  ...
  double **x = atom->x;
  double **f = atom->f;
  double *rad = atom->radius; //add in a particle radius dependence. requires particles of type sphere so there is a radius
  int *type = atom->type;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
	  ...
	  rsq = rsq/((rad[i]+rad[j])*(rad[i]+rad[j])); //rescale the distance by the average diameter (= sum of radii)
      jtype = type[j];
